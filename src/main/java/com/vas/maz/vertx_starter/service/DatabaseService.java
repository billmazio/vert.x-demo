...
//          // Insert the new user into the database
          ...
          JsonArray params = new JsonArray().add(user.getUsername()).add(hashedPassword);
  ...
  public void getUserPassword(String username, String providedPassword, Handler<AsyncResult<Boolean>> resultHandler) {
    ...
    jdbcClient.querySingleWithParams(query, new JsonArray().add(username), ar -> {
      if (ar.succeeded() && ar.result() != null) {
        ...
        boolean authenticationSuccess = BCrypt.checkpw(providedPassword, storedPassword);
        ...
        // Query failed
        logger.error("Failed to retrieve user password", ar.cause());
    ...
    jdbcClient.querySingleWithParams(checkUserQuery, checkParams, checkUser -> {
      if (checkUser.failed()) {
        ...
        if (checkUser.result() != null) {
          ...
          String updateUserQuery = "UPDATE users SET username = ? WHERE id = ?";
          JsonArray params = new JsonArray().add(user.getUsername()).add(user.getId());
...
//
//  private JDBCClient jdbcClient;
//  private final ConcurrentLinkedQueue<Consumer<Handler<AsyncResult<Void>>>> queue = new ConcurrentLinkedQueue<>();
//  private boolean isProcessing = false;
//
//  public DatabaseService(JDBCClient jdbcClient, Vertx vertx) {
//    this.jdbcClient = jdbcClient;
//
//    // Process the queue periodically
//    vertx.setPeriodic(1000, id -> processQueue());
//  }
//
//  private void processQueue() {
//    if (!isProcessing && !queue.isEmpty()) {
//      isProcessing = true;
//      Consumer<Handler<AsyncResult<Void>>> operation = queue.poll();
//      operation.accept(this::onOperationComplete);
//    }
//  }
//
//  private void onOperationComplete(AsyncResult<Void> result) {
//    if (result.failed()) {
//      logger.error("Operation failed", result.cause());
//    }
//    isProcessing = false;
//  }
//
//  // Queueing the operations
//  public void enqueueOperation(Consumer<Handler<AsyncResult<Void>>> operation) {
//    queue.offer(operation);
//  }
//
//  public void saveUser(User user, Handler<AsyncResult<Void>> resultHandler) {
//    enqueueOperation(handler -> {
//      String checkUserQuery = "SELECT id FROM users WHERE username = ?";
//      jdbcClient.querySingleWithParams(checkUserQuery, new JsonArray().add(user.getUsername()), checkUser -> {
//        if (checkUser.failed()) {
//          handler.handle(Future.failedFuture(checkUser.cause()));
//        } else {
//          if (checkUser.result() != null) {
...
//    enqueueOperation(handler -> {
//      if (user == null || user.getId() == null) {
//        handler.handle(Future.failedFuture("Invalid user or user id"));
//        return;
//      }
//      String checkUserQuery = "SELECT id FROM users WHERE username = ? AND id <> ?";
//      JsonArray checkParams = new JsonArray().add(user.getUsername()).add(user.getId());
...
//        } else {
//          if (checkUser.result() != null) {
//            handler.handle(Future.failedFuture("New username already exists"));
//          } else {
//            String updateUserQuery = "UPDATE users SET username = ? WHERE id = ?";
//            JsonArray params = new JsonArray().add(user.getUsername()).add(user.getId());
//            jdbcClient.updateWithParams(updateUserQuery, params, updateResult -> {
